...
if(isset($_GET['id'])){
    ...
    $thumbnails_result = mysqli_query($connnection,$thumbnails_query);

    if(mysqli_num_rows($thumbnails_result) > 0){
        while($thumbnail = mysqli_fetch_assoc($thumbnails_result)){
            $thumbnail_path = '../images/' . $thumbnail['thumbnail'];
            // delete thumbnail form images folder if exist
            if($thumbnail_path){
    ...
    $delete_user_result = mysqli_query($connnection,$delete_user_query);
